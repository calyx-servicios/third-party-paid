                    ...
                    if (activePoint._chart.data.domains) {
                        ...
                        if (item_data.max_sequnce != 0 && sequnce < item_data.max_sequnce) {
                            ...
                            if (item_data.ks_show_records) {
                                if(this.getSession().user_companies.allowed_companies){
                                    var ks_allow_company_ids = []
                                        ...
                                        if(odoo.session_info.user_context.ks_allow_company_ids === undefined){
                                            odoo.session_info.user_context.ks_allow_company_ids = odoo.session_info.user_context.allowed_company_ids;
                                        }
                                        odoo.session_info.user_context.allowed_company_ids = ks_allow_company_ids;
                                    }
            ...
            for (var i = 0; i < ids.length; i++) {
                ...
                if (item_data['ks_dashboard_item_type'] == 'ks_list_view') {
                    ...
                    if (ks_length) {
                        ...
                        if (item_data.ks_record_count <= item_data.ks_pagination_limit) item_view.find('.ks_load_next').addClass('ks_event_offer_list');
                        item_view.find('.ks_value').text("1-" + JSON.parse(item_data['ks_list_view_data']).data_rows.length);
                    } else {
                        item_view.find('.ks_pager').addClass('d-none');
                    }
...
//                console.log("test1");
//                _.each(notifications, (function (notification) {
//                    if (notification[0][1] === 'ks_dashboard_ninja.notification') {
//                        this.display_calendar_notif(notification[1]);
//                        self.call('notification', 'notify', {
//                            message: "You can only chat with employees that have a dedicated user.",
//                            type: 'info',
//                        });
//                    }
//                }).bind(this));
