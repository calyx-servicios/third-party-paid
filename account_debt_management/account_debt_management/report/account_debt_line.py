...
class AccountDebtLine(models.Model):
    ...
    def _compute_move_lines_data(self):
        ...
        for rec in self:
            ...
            move_lines = rec.move_line_ids.search([("id", "in", move_lines_str)])
            ...
            if len(move_lines) == 1:
                ...
                rec.move_line_id = (
                    len(move_lines) == 1
                    and move_lines[0]
                    or rec.env["account.move.line"]
                )

            invoice_id = rec.move_line_ids.mapped("move_id")
            rec.invoice_id = len(invoice_id) == 1 and invoice_id

            payment_group = rec.move_line_ids.mapped("payment_id.payment_group_id")
            ...
            rec.financial_amount = sum(rec.move_line_ids.mapped("financial_amount"))
    ...
    def cancel_amount_residual_currency(self):
        ...
        self = self.with_context(default_ref=_("Ajuste manual de deuda en divisa"))
        for aml in self.mapped("move_line_ids"):
            if not float_is_zero(
                aml.amount_residual, precision_rounding=aml.company_currency_id.rounding
            ):
                raise UserError(
                    _(
            ...
            partial_rec = (
                (aml.credit and aml.matched_debit_ids)
                and aml.matched_debit_ids[0]
                or aml.matched_credit_ids
                and aml.matched_credit_ids[0]
                or False
            )
            ...
            if partial_rec:
                ...
                ).create_exchange_rate_entry(aml, exchange_move)
            exchange_move.post()
