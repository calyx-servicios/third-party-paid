...
class MailActivity(models.Model):
    ...
    sh_state = fields.Selection(related='state', store=True,string="state")
...
# FILTER USER
    ...
    def get_sh_crm_activity_all_tbl(self, filter_date, filter_user, start_date,
                                    ...
                                    filter_model, filter_record,filter_type):
        doman = []
        if filter_type and filter_type != None:
            doman.append(('activity_type_id','=',int(filter_type)))
        crm_days_filter = filter_date
        ...
        if filter_model not in ["", None, False]:
            ...
            if model_id:
                ...
                document_model_records = self.env[
                    model_id.model].sudo().search_read(domain,['id','name'])
